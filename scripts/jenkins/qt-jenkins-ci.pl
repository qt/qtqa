...
Default: generated by the summarize-jenkins-build.pl script, with
a list of tested changes appended
...
#   stdout => scalar ref to receive stdout of the process
...
sub jenkins_build_summary
{
    ...
    if (!$self->{ cfg }{ jenkins_build_summary }) {
        ...
        my $tested_changes = eval { $self->tested_changes_summary() };
        if (my $error = $EVAL_ERROR) {
            $tested_changes = "(error while determining tested changes: $error)";
        }

        # indent under a 'Tested changes' header
        $tested_changes =~ s{^}{    }mg;
        $tested_changes = "  Tested changes:\n$tested_changes";
...
sub tested_changes_summary
{
    ...
    if (!$self->{ tested_changes_summary }) {
        my @links;
        ...
        my $gerrit_link_prefix = 'https://' . $self->gerrit_host();
        ...
        foreach my $line (@lines) {
            if ($line =~
                ...
                push @links, "$gerrit_link_prefix/$1 [patch set $2] - $3";
            } else {
                # If we can't parse the output for whatever reason, still stick it into the
                # summary, to make sure the problem is noticed.
                push @links, "(unknown change - $line)";
            }
        }

        $self->{ tested_changes_summary } = join( "\n", @links );
    }

    return $self->{ tested_changes_summary };
}
...
sub do_staging_new_build
{
    ...
    do_robust_cmd(
        stdout => \$output,
        cmd => ['git', 'ls-remote', $self->gerrit_git_url( ), $build_ref],
        retry_exitcodes => [255],
    );
    if (!$output) {
